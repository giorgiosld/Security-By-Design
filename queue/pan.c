...
/*** From source: /home/giorgiosld/university/master/SBD/lab/abpunreliablechannel.pml ***/
...
short src_ln2 [] = {
	  0,  54,  56,  58,  55,  60,  53,  61, 
	 53,  61,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
	{ "/home/giorgiosld/university/master/SBD/lab/abpunreliablechannel.pml", 1, 9 },
	{ "-", 10, 11 }
};
uchar reached2 [] = {
	  0,   1,   1,   1,   0,   1,   0,   1, 
...
short src_ln1 [] = {
	  0,  42,  44,  45,  45,  46,  43,  48, 
	 41,  49,  41,  49,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "/home/giorgiosld/university/master/SBD/lab/abpunreliablechannel.pml", 1, 11 },
	{ "-", 12, 13 }
};
uchar reached1 [] = {
	  0,   1,   1,   1,   0,   0,   0,   1, 
...
short src_ln0 [] = {
	  0,  18,  21,  22,  20,  24,  24,  27, 
	 27,  28,  29,  29,  30,  31,  32,  28, 
	 34,  26,  35,  17,  36,  17,  36,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "/home/giorgiosld/university/master/SBD/lab/abpunreliablechannel.pml", 1, 22 },
	{ "-", 23, 24 }
};
uchar reached0 [] = {
	  0,   1,   1,   1,   0,   1,   0,   1, 
...
{	int j = 0, h = now._nr_pr;
	...
	case 1:	/* Receiver */
		...
		((P1 *)pptr(h))->y = 0;
#ifdef VAR_RANGES
		logval("Receiver:altbit", ((P1 *)pptr(h))->altbit);
	...
	case 0:	/* Sender */
		...
		((P0 *)pptr(h))->x = 0;
#ifdef VAR_RANGES
		logval("Sender:altbit", ((P0 *)pptr(h))->altbit);
...
{
		now.e = addqueue(calling_pid, 1, 1);
		now.f = addqueue(calling_pid, 2, 1);
		now.c = addqueue(calling_pid, 3, 0);
		now.d = addqueue(calling_pid, 4, 0);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.e) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.e) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.e) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.e) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.f) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.f) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.f) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.f) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.c) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.c) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.c) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.c) )))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.d) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.d) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.d) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.d) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  timer_on:	3\n");
	printf("	mtype  timer_off:	2\n");
	printf("	mtype  tout:	1\n");
	printf("	chan e (=%d):	len %d:\t", now.e, q_len(now.e));
	c_chandump(now.e);
	printf("	chan f (=%d):	len %d:\t", now.f, q_len(now.f));
	c_chandump(now.f);
	printf("	chan c (=%d):	len %d:\t", now.c, q_len(now.c));
	c_chandump(now.c);
	printf("	chan d (=%d):	len %d:\t", now.d, q_len(now.d));
	c_chandump(now.d);
}
void
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("tout"); return;
	case 2: Printf("timer_off"); return;
	case 3: Printf("timer_on"); return;
